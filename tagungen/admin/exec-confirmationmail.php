...
if ($_POST['confirmDate'] == "prioThun"){
    $sql = "UPDATE `contact_form_information_events_test` SET `Status` = 'Bestätigt', `confAtendenceLocation` = '25.1.19 Thun' WHERE `id` = ".$_POST['id'];
    $result = $conn->query($sql);
    
}elseif ($_POST['confirmDate'] == "prioWinterthur"){
    $sql = "UPDATE `contact_form_information_events_test` SET `Status` = 'Bestätigt', `confAtendenceLocation` = '25.1.19 Winterthur' WHERE `id` = ".$_POST['id'];
    $result = $conn->query($sql);
    
}elseif ($_POST['confirmDate'] == "prioZofingen"){
    $sql = "UPDATE `contact_form_information_events_test` SET `Status` = 'Bestätigt', `confAtendenceLocation` = '11.1.19 Zofingen' WHERE `id` = ".$_POST['id'];
    $result = $conn->query($sql);
    
}elseif ($_POST['confirmDate'] == "altThun"){
    $sql = "UPDATE `contact_form_information_events_test` SET `Status` = 'Umgebucht', `confAtendenceLocation` = '25.1.19 Thun' WHERE `id` = ".$_POST['id'];
    $result = $conn->query($sql);
    
}elseif ($_POST['confirmDate'] == "altWinterthur"){
    $sql = "UPDATE `contact_form_information_events_test` SET `Status` = 'Umgebucht', `confAtendenceLocation` = '25.1.19 Winterthur' WHERE `id` = ".$_POST['id'];
    $result = $conn->query($sql);
    
}elseif ($_POST['confirmDate'] == "altZofingen"){
    $sql = "UPDATE `contact_form_information_events_test` SET `Status` = 'Umgebucht', `confAtendenceLocation` = '25.1.19 Zofingen' WHERE `id` = ".$_POST['id'];
    $result = $conn->query($sql);
    
}else {
    exit("Konnte Datum nicht bestätigen.");
}
...
$lang  = $myArray[0]['detectedLang']  ; // beachte das Komma
// Betreff
...
<body style="font-size:11.0pt;font-family:&quot;Verdana&quot;,&quot;sans-serif&quot;; ">
    Bonjour ' . $myArray[0]['firstname'] . " " . $myArray[0]['lastname']  . ',
...
<body style="font-size:11.0pt;font-family:&quot;Verdana&quot;,&quot;sans-serif&quot;; ">
    <p>Guten Tag ' . $myArray[0]['firstname'] . " " . $myArray[0]['lastname']  . '</p>
...
// zusätzliche Header
$header[] = 'To: ' . $myArray[0]['firstname'] . '' . $myArray[0]['lastname'] . '<' . $myArray[0]['eMail'] . '>';
$header[] = 'From: Regina Schellenberg (Sekretariat) <marius.baumann@emk-schweiz.ch>';
$header[] = 'Cc: Regina Schellenberg (Sekretariat) <marius.baumann@emk-schweiz.ch>';
